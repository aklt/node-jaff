    ...
    if (!sandbox) {
        sandbox = {
            ...
            __quotes: function (q1, q2) {
                that.context.__expand = [q1, q2];
                // to propagate into env
                // that.runScript('__expand');
                that._configure();
                return '';
            },
            encodeURI: encodeURI,
        ...
        for (var method in global) {
            sandbox[method] = global[method];
        }
    }
    sandbox.__filename = basename(this.fileName);
    sandbox.__dirname = dirname(this.fileName) || process.cwd();
...
MacroTransform.prototype._transform = function (line, buffer, cb) {
    ...
    if (m) {
        this.context.__language = m[1] === '#' ? 'coffee' : 'js';
        if (m[2]) {
...
MacroTransform.prototype.____action____end = function (arg, cb) {
    ...
    if (obj.id === BEGIN) {
        this.context.__language = (obj.arg  === 'coffee') ? 'coffee' : 'js';
        var script = obj.lines.join('');
        if (this.context.__language === 'coffee') {
        ...
        if (obj.cmd[0] === '!') {
            ...
            this.context.$tmp = obj.lines;
            var data = that.runScript(filter + '($tmp);');
            delete this.context.$tmp;
    ...
    findFileInParentDirs(fileName, dirName, function (err, filePath) {
        ...
        var stream = fs.createReadStream(filePath),
            macroStream = MacroTransform.fromStream(
                that.context, stream, dirName, filePath, parents);
...
MacroTransform.prototype.____action____with = function (variableName, cb) {
    ...
    this.context[variableName] = variableType === 'object' ? {} : [];
...
MacroTransform.prototype.____action____do = function (empty, cb) {
    ...
    this.context[obj.variable] = result;
    obj.atDo = true;
    cb();
};
...
MacroTransform.prototype._configure = function () {
    var begin = this.context.__expand[0],
        end = this.context.__expand[1];
