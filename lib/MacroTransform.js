...
MacroTransform.prototype.runScript = function (script, newContext) {
    ...
    try {
        if (newContext) return vm.runInNewContext(script, this.sandbox, fileName);
        else return vm.runInContext(script, this.context, fileName);
    } catch (e) {
        this.log('warn', e);
    }
};
...
// Expand multi-char code as in the line 
// This is {{foobar}} and here is {{Gooble}}.
MacroTransform.prototype.expandMultiCharQuotes = function (code, keys) {
    var it = code,
    ...
    do {
        ...
        if (index < 0) {
            return Buffer.concat(result, it);
        }
        var content = it.slice(0, index);
        it = it.slice(index + endLen);
        var value = keys[content];
